...
export const copy = {
  en: {
    ...
    phoneCaption: "The current unified task list and composer across Cursor, Claude Code, Codex, Copilot, and Grok Build, shown with deterministic demo data.",
    ...
    chatCaption: "The chat shows one delivered photo, one agent reply, and a single keyboard/send control that stays in one place.",
    photoCaption: "A photo from your own outgoing delivery opens locally with zoom and pan. Attachments without bytes stay visibly non-interactive.",
    ...
    securityCaption: "The current on-device settings for connections, per-agent answer defaults, language, and approval behavior, shown with deterministic demo data.",
    installKicker: "Public and auditable machine bridge",
    installTitle: "Connect the agents you already use.",
    installText:
      "One GrantTap installation owns the encrypted machine pairing. Setup adds every detected agent adapter to it; installing for another agent never creates a second helper or phone pairing.",
  ...
  ru: {
    ...
    phoneCaption: "Текущая единая лента задач и композер для Cursor, Claude Code, Codex, Copilot и Grok Build с детерминированными демо-данными.",
    ...
    chatCaption: "В чате видны одно доставленное фото, один ответ агента и единая кнопка клавиатуры/отправки, которая остаётся на месте.",
    photoCaption: "Фото из собственной отправки открывается локально с зумом и перемещением. Вложения без байтов явно не выглядят активными.",
    ...
    securityCaption: "Текущие настройки подключений, ответов отдельных агентов, языка и подтверждений с детерминированными демо-данными.",
    installKicker: "Публичный и проверяемый мост",
    installTitle: "Подключите агентов, которыми уже пользуетесь.",
    installText:
      "Одна установка GrantTap хранит зашифрованный пейринг компьютера. Setup добавляет к ней все найденные адаптеры агентов; подключение другого агента не создаёт вторую установку или пейринг телефона.",
